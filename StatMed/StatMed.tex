...
\begin{document}
	...
	where $F(\bd{x}, \bd{z})$ is the log odds function. Maximizing the probability is equivalent to minimizing the following log loss function:
	...
	\end{equation}
	where $h_0(t)$ is an unknown baseline hazard function. Using the partial likelihood function enables us to circumvent the inference of $h_0(x)$, and directly estimate $F(\bd{x}, \bd{z})$. We use the negative log partial likelihood as the loss function:
	...
	where $\bd{F} = (F(\bd{x}_1, \bd{z}_1), F(\bd{x}_2, \bd{z}_2), \ldots, F(\bd{x}_N, \bd{z}_N))$. In the rest of this chapter, we will also use boldface font of a function to represent the vector of the function evaluated at each observed sample point.
	
	\subsection{Identify the optimal increment function} \label{sec:identify}
	Boosting is an iterative functional descent procedure to minimize the empirical loss function. The most crucial step in the Boosting procedure is the identification of an ``optimal" increment function $f \in \mathcal{G}$, which shrinks the empirical loss as much as possible, given the current value of $F(\bd{x}, \bd{z})$. We approximate $L(\bd{y},\bd{F})$ by its second order Taylor expansion around $\bd{F}$:
	\begin{eqnarray*}
		L(\bd{y}, \bd{F} + \bd{f}) &\approx& L(\bd{y}, \bd{F}) +  (\grad{L}{F})^T \bd{f} + \frac{1}{2}\bd{f}^T \hess{F} \bd{f} \\
		& = & \frac{1}{2} (\bd{f} + \hess{F}^{-1} \grad{L}{F})^T \hess{F} (\bd{f} + \hess{F}^{-1} \grad{L}{F}) + \mbox{const},
	\end{eqnarray*}
where $f$ is the increment function. $\nabla_{\bd{F}} L$ and $\bd{H}_{\bd{F}}$ are the gradient and Hessian matrix of $L(\bd{y}, \bd{F})$ with respect to $\bd{F}$, respectively, and const includes all the terms that do not involve $\bd{f}$. Note that this approximation is accurate in the case of regression, since the regression loss is quadratic itself. Calculations of $\grad{L}{F}$ and $\hess{F}$ for different problems are available in Appendix A in the Supplementary Materials. Direct minimization for this loss approximation in $\mathcal{G}$, however, may lead to overfitting, due to the high flexibility of the base learner space. It is necessary to apply penalties in the selection of $f$. Therefore we propose the following regularized loss as the working loss function in PKB:
$$L_{R}(f) = \frac{1}{2} (\bd{f} + \hess{F}^{-1} \grad{L}{F})^T \hess{F} (\bd{f} + \hess{F}^{-1} \grad{L}{F}) + \lambda \Omega(f),$$
where $\Omega(f)$ is the penalty term. Since $f$ takes the functional form as presented in (\ref{eqn:G}), it is natural to consider the  $L_1$ and $L_2$ norm of $\beta$ as choices of penalty: $\Omega(f) = \| \beta \|_1$ or $\| \beta \|_2^2$. 

Optimizing $L_R(f)$ in $\mathcal{G}_m$ is equivalent to
\begin{equation}
\label{eqn:regloss}
...
\end{eqnarray*}
solving $\beta$ in problem (\ref{eqn:regloss}) is reduced to:
\begin{equation}
\label{eqn:reduced}
\min_{\beta} \|\tilde{\eta} + \tilde{K}_m \beta \|_2^2 + \lambda \Omega(f).
\end{equation}
Proof is provided in Appendix B in the Supplementary Materials. Problem (\ref{eqn:reduced}) becomes the LASSO problem when we use the $L_1$ penalty, and the Ridge Regression when we use $L_2$ penalty. Both can be efficiently solved with existing solvers. After solving $\beta$, we can subsequently obtain the solution of $\gamma$ as:
...
\begin{enumerate}
	...
	\item Identify the optimal increment function $f$\\
	At iteration $t$, we calculate the gradient $\grad{L}{F_t}$ and Hessian matrix $\hess{F_t}$. For each pathway $m$, we solve the optimal $\hat{f}_m \in \mathcal{G}_m$ and corresponding $\beta, \gamma$ following the steps in Section \ref{sec:identify}. The optimal increment function $\hat{f}$ is the $\hat{f}_m$ which yields the smallest $L_R(f)$.
	\item Line search and update $F_t$\\
	While $\hat{f}$ gives the direction in which the regularized loss has the steepest descent, we still need to decide the step length. We first perform a line search
	$$ \hat{d} = \arg\min_{d \in R^{+}} L(\bd{y}, \bd{F_t} + d\hat{\bd{f}}).$$
	The solution $\hat{d}$ represents the step size to reach the bottom of the empirical loss following the direction of $\hat{f}$. We then apply a shrinkage on the step size using a learning rate parameter $\nu$, and update our estimate of the score function:
	$$F_{t+1}(\bd{x}, \bd{z}) = F_t(\bd{x}, \bd{z}) + \nu\hat{d} \hat{f}(\bd{x}, \bd{z}).$$
...
In this section, we apply PKB to a variety of simulation datasets, and demonstrate that it can yield better prediction accuracy compared to competing methods, as well as correctly identify informative pathways.
